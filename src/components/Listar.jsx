...
const Listar = ({estado, setIdmetro}) => {
    ...
    const handleDelete = async (id) => {
        try {
            ...
            if (confirmar) {
                const url = `https://65b819a946324d531d55f1bd.mockapi.io/metro/${id}`
                await fetch(url, {
                    method: 'DELETE',
                })
    ...
    useEffect(() => {
        //Verificar si la varible estado se ha cambiado 
        ...
        if(estado || rutas.length>=0)
        {
            (async function () {
                try {
                    ...
                    const respuesta = await (await fetch("https://65b819a946324d531d55f1bd.mockapi.io/metro")).json()
                    //Asignar el resultado a la varible setRutas
                    setRutas(respuesta)
